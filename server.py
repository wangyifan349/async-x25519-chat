...
def main():
    ...
    local_private_key, local_public_key = generate_ecdh_key_pair()         # Server's static key pair
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_sock:
        ...
        # 2. Exchange compressed public keys (33 bytes) ← insecure but enough to derive secrecy if no MITM
        conn.sendall(local_public_key.format(compressed=True))             # Send our pubkey first
        remote_public_bytes = recv_exact(conn, 33)                         # Wait for their pubkey
        ...
        print("Secure session key established.")                           # If this key is secret, channel has forward secrecy
        # 4. Start parallel threads for independent send/recv
        ...
        # 5. Wait forever (threads persist)
